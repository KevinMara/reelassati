...
const STEPS = {
  en: [
    { num: "01", title: "Bring the material", description: "Upload footage, start from a brief, or capture a voice note. Add the audience and outcome before editing." },
    { num: "02", title: "Direct the cut", description: "Work manually on the timeline or ask for a change in plain language. Select and lock any range that must stay untouched." },
    { num: "03", title: "Review every proposal", description: "See what will change, where, and why. Accept individual edits, compare a version, or keep your original." },
    { num: "04", title: "Finish and deliver", description: "Run a quality preflight, export the right format, or publish after connecting an authorized platform account." },
  ],
  it: [
    { num: "01", title: "Porta il materiale", description: "Carica il girato, parti da un brief o registra una nota vocale. Definisci pubblico e risultato prima del montaggio." },
    { num: "02", title: "Dirigi il montaggio", description: "Lavora manualmente sulla timeline o chiedi una modifica in linguaggio naturale. Seleziona e blocca ciò che non deve cambiare." },
    { num: "03", title: "Revisiona ogni proposta", description: "Vedi cosa cambierà, dove e perché. Accetta singole modifiche, confronta una versione o conserva l’originale." },
    { num: "04", title: "Finalizza e consegna", description: "Esegui il preflight qualità, esporta nel formato giusto o pubblica dopo aver collegato un account autorizzato." },
  ],
} as const;
...
export function HowItWorks() {
  ...
  return (
    <section ref={ref} className="border-y border-border bg-surface/40 py-24 md:py-36">
      <div className="container-page">
        <div className="max-w-3xl">
          <p className="mono-eyebrow mb-4 text-primary">{isItalian ? "Dal girato alla versione finale" : "From footage to final"}</p>
          <h2 className="text-display-md font-semibold">
            {isItalian ? "Assistenza dove serve." : "Assistance where it helps."}{" "}
            <span className="serif-accent">{isItalian ? "Controllo dove conta." : "Control where it matters."}</span>
          </h2>
        </div>
        ...
        <div className="mx-auto mt-16 w-full max-w-4xl rounded-xl border border-border-strong bg-surface p-4 shadow-cutout md:p-6">
          ...
          <div className="relative mt-5 space-y-2 overflow-hidden rounded-lg border border-border bg-surface-recessed/90 p-3">
            ...
            <Track label={isItalian ? "Video" : "Video"} widths={["32%", "19%", "38%"]} />
        ...
        >
          {steps.map((step) => (
            <motion.article key={step.num} variants={fadeUp} className="relative bg-background p-8 md:p-9">
              <div className="mb-6 flex items-baseline justify-between">
                <span className="font-mono text-xs uppercase tracking-wider text-primary">{step.num}</span>
                <span className="mono-eyebrow text-foreground/30">{isItalian ? "passo" : "step"}</span>
              </div>
