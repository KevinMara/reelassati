...
export default function TemplatesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return VIDEO_PROMPT_TEMPLATES.filter(template => {
      ...
      return `${template.name} ${template.description} ${template.preview}`
        .toLowerCase()
        .includes(query);
    });
  ...
  const selected =
    filtered.find(template => template.id === selectedId) ?? filtered[0];
  const compiledPrompt = selected?.buildPrompt(
    subject.trim() ||
      (language === "it" ? "Descrivi il soggetto." : "Describe the subject.")
  );
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-10">
            ...
            <h1 className="text-4xl font-semibold md:text-5xl">
              {language === "it"
                ? "Preset produttivi, non formule virali."
                : "Production presets, not viral formulas."}
            </h1>
          ...
          <div className="mb-8 flex flex-wrap items-center gap-3">
            <label className="relative min-w-[240px] flex-1 sm:max-w-sm">
              <span className="sr-only">
                {language === "it" ? "Cerca preset" : "Search presets"}
              </span>
              <Search
            ...
            >
              {categories.map(category => {
                const label =
                  category === "all"
                    ? ALL_LABEL[language]
                    : CATEGORY_META[category].label[language];
                return (
                  <button
                    key={category}
                    type="button"
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr_0.92fr]">
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className="rounded-pill bg-background px-2.5 py-1 font-mono text-[9px] uppercase tracking-wider text-foreground/45">
                            {template.defaultRatio} · {template.defaultDuration}
                            s
                          </span>
                        </div>
            ...
            {selected ? (
              <aside
                className="h-fit rounded-2xl border border-border bg-surface p-6 shadow-card lg:sticky lg:top-24 md:p-7"
                aria-label={
                  language === "it"
                    ? "Compiler prompt locale"
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="mono-eyebrow text-primary">
                      {language === "it" ? "Compiler locale" : "Local compiler"}
                    </p>
                ...
                <div className="mt-5 rounded-xl border border-primary/20 bg-primary/[0.04] p-4">
                  ...
                  <p className="mt-3 max-h-64 overflow-y-auto text-xs leading-relaxed text-foreground/70">
                    {compiledPrompt}
                  </p>
                </div>
                ...
                <Link
                  to={`/dashboard/video?template=${encodeURIComponent(
                    selected.id
                  )}&subject=${encodeURIComponent(subject.trim().slice(0, 1200))}`}
