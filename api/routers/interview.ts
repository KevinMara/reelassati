...
Return ONLY JSON: [{"id":1,"question":"..."},{"id":2,"question":"..."},...]`;
      ...
      const response = await chatCompletion([
        { role: "system", content: "You are a skilled podcast interviewer who helps creators share their authentic stories." },
        { role: "user", content: prompt },
      ]);

      let questions: any[] = [];
      try { if (response) questions = JSON.parse(response); } catch { /* ignore */ }

      // Fallback
      if (!questions.length) {
        questions = [
          { id: 1, question: `What first got you interested in ${input.topic}?` },
          { id: 2, question: `What's the biggest misconception people have about ${input.topic}?` },
          { id: 3, question: `Can you share a specific story or moment that changed everything for you?` },
          { id: 4, question: `What advice would you give someone just starting out with ${input.topic}?` },
          { id: 5, question: `What's one thing you wish you knew when you started?` },
        ];
      }
  ...
  generate: publicQuery
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await db.update(interviewSessions).set({ status: "generating" })
        .where(eq(interviewSessions.id, input.sessionId));

      const session = await db.select().from(interviewSessions)
        .where(eq(interviewSessions.id, input.sessionId)).limit(1);
      if (!session[0]) throw new Error("Session not found");

      const qa = (session[0].questions as any[]).map((q: any) => {
        const a = (session[0].answers as any[]).find((ans: any) => ans.questionId === q.id);
...
Return ONLY JSON: [{"hook":"...","body":"...","cta":"...","platform":"tiktok"},{"hook":"...","body":"...","cta":"...","platform":"instagram"},{"hook":"...","body":"...","cta":"...","platform":"youtube"}]`;
      ...
      const response = await chatCompletion([
        { role: "system", content: "You are a viral content creator who turns interviews into engaging short-form posts." },
        { role: "user", content: prompt },
      ]);

      let content: any[] = [];
      try { if (response) content = JSON.parse(response); } catch { /* ignore */ }

      if (!content.length) {
        content = [{ hook: "You won't believe this...", body: "Based on my interview about " + session[0].topic, cta: "Follow for more!", platform: "tiktok" }];
      }
