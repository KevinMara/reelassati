...
serve(async (req) => {
  ...
  try {
    ...
    const { video_id, client_id, quality, instructions } = await req.json()
    
    const estimatedCost = quality === 'high' ? 0.65 : 0.20
    ...
    const { data: job, error } = await supabase.from('jobs').insert({
      ...
      status: 'queued',
      payload: { video_id, quality, instructions },
    ...
    try {
      await invokeAgent({
        ...
        input: { video_id, quality, instructions, pipeline: ['TRIBE_v2', 'Opus_4.7', 'GPT_Image_2'] },
